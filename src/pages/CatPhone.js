...
import PhoneSection from "../components/categories/CategoriesPhone";
import { Box, useTheme, FormControl, InputLabel, Select, MenuItem, Button, Grid, Typography, Divider, Paper, Container, Fade, Grow, Chip } from "@mui/material";
...
const CatPhone = ({ darkMode, toggleDarkMode }) => {
    ...
    const [filters, setFilters] = useState({
        categoryId: 4, // Phone category ID
        color: '',
        storageGb: '',
        ramGb: '',
        display: '',
    ...
    const fetchFilterOptions = async () => {
        try {
            const response = await api.get("/product/filter/options", {
                params: { categoryId: 4 } // Phone category ID
            });
    ...
    const fetchFilteredProducts = async () => {
        ...
        try {
            ...
            try {
                ...
                const fallbackResponse = await api.get("/product/category/1");
                        ...
                        <Container maxWidth="xl" sx={{ mb: 4, pt: 2 }}>
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        Манай сүүлийн үеийн ухаалаг гар утаснуудаас сонголтоо хийнэ үү
                                    </Typography>
                                </Box>
                            </Box>
                        </Container>
                            ...
                            <Grid container spacing={3}>
                                <Grid item xs={12}>
                                    <Box sx={{
                                        ...
                                        {activeFiltersCount > 0 && (
                                            <Chip
                                                label={`${activeFiltersCount} идэвхтэй шүүлтүүр${activeFiltersCount > 1 ? '' : ''}`}
                                                size="small"
                                                color="primary"
                                                variant="outlined"
                                                sx={{ fontWeight: 500 }}
                                ...
                                <Grid item xs={12}>
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
                                        {['color', 'storageGb', 'ramGb', 'display', 'processor'].map(filter => {
                                ...
                                <Grid item xs={12} sm={6} md={2}>
                                    <FormControl fullWidth size="small" sx={{ mb: { xs: 1, md: 0 } }}>
                                        ...
                                        >
                                            <MenuItem value="">Бүх багтаамж</MenuItem>
