...
const Header = ({ darkMode, toggleDarkMode }) => {
  ...
  const checkUserAuth = () => {
    ...
    console.log('Auth check - Names:', storedFirstName, storedLastName);
    
    if (token && (storedFirstName || storedLastName)) {
      setIsLoggedIn(true);
      setFirstName(storedFirstName || '');
      setLastName(storedLastName || '');
      console.log('User authenticated:', storedFirstName, storedLastName);
    } else {
      setIsLoggedIn(false);
      setFirstName('');
      setLastName('');
  ...
  const updateCartCount = async () => {
    try {
      ...
      setCartCount(0); // Reset to 0 if there's an error
    }
  };
  ...
  const searchProducts = async (keyword) => {
    try {
      ...
      const response = await api.get(`http://localhost:8083/product/search?keyword=${encodeURIComponent(keyword)}`);
