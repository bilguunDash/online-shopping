...
const CategoriesPc = ({ products, loading, error, darkMode }) => {
  ...
  const [loginPromptMessage, setLoginPromptMessage] = useState('Үргэлжлүүлэхийн тулд нэвтэрнэ үү');
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!isLoggedIn) {
      handleRequireLogin('Хүслийн жагсаалтад нэмэхийн тулд нэвтэрнэ үү');
      return;
    }
    ...
    if (isInWishlist) {
      ...
      showFeedback(`${product.title || product.name} хүслийн жагсаалтаас хасагдлаа!`, 'info');
    } else {
      // Create a clean product object with all necessary fields
      const cleanProduct = {
        id: productId,
        productId: productId,
        name: product.name || product.title || "Бүтээгдэхүүн",
        title: product.title || product.name || "Бүтээгдэхүүн",
      ...
      showFeedback(`${product.title || product.name} хүслийн жагсаалтад нэмэгдлээ!`, 'success');
    }
  ...
  const handleAddToCart = async (e, product) => {
    ...
    // Check if user is authenticated
    if (!isLoggedIn) {
      handleRequireLogin('Сагсанд нэмэхийн тулд нэвтэрнэ үү');
      return;
    }
    ...
    try {
      ...
      showFeedback(response.data?.message || `${product.title || product.name} сагсанд нэмэгдлээ!`);
      ...
      const errorMessage = err.response?.data?.message || "Сагсанд нэмэх үйлдэл амжилтгүй боллоо. Дахин оролдоно уу.";
      ...
      if (errorMessage.includes('stock') || errorMessage.includes('нөөц')) {
        ...
        showFeedback(errorMessage, "info");
      } else {
      ...
      <Container maxWidth="xl" sx={{ pt: 3, pb: 6 }}>
        {loading ? (
          ...
          <Box sx={{ 
            ...
            <DesktopWindowsIcon sx={{ fontSize: 64, color: darkMode ? '#aaa' : '#666', mb: 2 }} />
          ...
          <>
            <Box sx={{ 
              ...
              <DesktopWindowsIcon sx={{ mr: 2, fontSize: 32, color: '#6039f0' }} />
              <Typography 
                variant="h4" 
                component="h1" 
                sx={{ 
                    ...
                    >
                      ...
                      <Box sx={{ 
                        ...
                        <Box sx={{ mb: 2 }}>
                          ...
                          {!product.specs && (
                            <>
                              <Typography variant="body2" sx={{ fontSize: '0.75rem', color: darkMode ? '#bbb' : '#666', mb: 0.5 }}>
                                • {product.processor || 'Intel Core i7'} 
                              </Typography>
                              <Typography variant="body2" sx={{ fontSize: '0.75rem', color: darkMode ? '#bbb' : '#666', mb: 0.5 }}>
                                • {product.ramGb || '16GB'} RAM
                              </Typography>
                              <Typography variant="body2" sx={{ fontSize: '0.75rem', color: darkMode ? '#bbb' : '#666', mb: 0.5 }}>
                                • {product.graphics || 'RTX 3060'} 
                              </Typography>
                            </>
                          )}
                        </Box>
