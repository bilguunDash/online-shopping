...
const CategoriesPhone = ({ products, loading, error, darkMode }) => {
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!isLoggedIn) {
      handleRequireLogin('Please log in to add items to your wishlist');
      return;
    }
  ...
  const handleAddToCart = async (e, product) => {
    ...
    try {
      ...
      showFeedback(response.data?.message || `${product.title || product.name} added to cart!`);
      ...
      <Container maxWidth="xl" sx={{ pt: 3, pb: 6 }}>
        {loading ? (
          ...
          <Box sx={{ 
            ...
            <SmartphoneIcon sx={{ fontSize: 64, color: darkMode ? '#aaa' : '#666', mb: 2 }} />
          ...
          <>
            <Box sx={{ 
              ...
              <SmartphoneIcon sx={{ mr: 2, fontSize: 32, color: '#6039f0' }} />
              <Typography 
                variant="h4" 
                component="h1" 
                sx={{ 
                          ...
                          {(!product.specifications || product.specifications.length === 0) && (
                            <>
                              <Typography variant="body2" sx={{ fontSize: '0.75rem', color: darkMode ? '#bbb' : '#666', mb: 0.5 }}>
                                • {product.ramGb || '4GB'} RAM
                              </Typography>
                              <Typography variant="body2" sx={{ fontSize: '0.75rem', color: darkMode ? '#bbb' : '#666', mb: 0.5 }}>
                                • {product.storageGb || '128GB'} хадгалалт
                              </Typography>
                              <Typography variant="body2" sx={{ fontSize: '0.75rem', color: darkMode ? '#bbb' : '#666', mb: 0.5 }}>
                                • {product.camera || '48MP'} камер
                              </Typography>
                            </>
                          )}
                        </Box> */}
